...
export class AddContractComponent {
  ...
  loadOffers() {
    this.http.get('http://localhost:8093/offer').subscribe((data: any) => {
      this.offers = data;
    });
  }

  loadJobSeekers() {
    this.http.get('http://localhost:8093/jobseeker').subscribe((data: any) => {
      this.jobSeekers = data;
    });
  }

  getEntrepriseId() {
    this.http.get('http://localhost:8093/authenticated-entreprise-id').subscribe((data: any) => {
